...
export function PannelloNotifiche({ iniziali }: { iniziali: Notifica[] }) {
  ...
  async function segnaTutte() {
    ...
    setNotifiche((precedenti) => precedenti.map((n) => ({ ...n, letta: true })));
  ...
  return (
    <section className="mt-14">
      <div className="flex items-baseline justify-between gap-4 border-b border-[var(--bordo)] pb-3">
        <Etichetta>
          Notifiche {nonLette > 0 && `· ${nonLette} nuove`}
        </Etichetta>
        {nonLette > 0 && (
          <button
            type="button"
            onClick={segnaTutte}
            className="mono text-[11px] uppercase tracking-[0.1em] text-[var(--testo-tenue)] transition-colors hover:text-[var(--accento)]"
          >
      ...
      <div>
        {notifiche.map((notifica, indice) => (
          <motion.div
            ...
            className="flex gap-4 border-b border-[var(--bordo)] py-4"
          >
            <span
              aria-hidden="true"
              className={`mt-1.5 h-1.5 w-1.5 shrink-0 ${
                notifica.letta ? "bg-[var(--bordo-forte)]" : "bg-[var(--accento)]"
              }`}
            />
            <div className="min-w-0 flex-1">
              ...
              <p className="mono mt-1 text-[12px] leading-[1.65] text-[var(--testo-tenue)]">
                {notifica.testo}
              </p>
            </div>
