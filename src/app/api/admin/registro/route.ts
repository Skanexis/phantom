 ...
 * ---
 *
 * Sulla paginazione, che è la scelta che decide se questa scheda resta
 * usabile fra sei mesi.
 *
 * Si scorre per **cursore** e non con `skip`. Con un OFFSET il database
 * deve comunque attraversare tutte le righe saltate: alla prima pagina non
 * si nota, alla ventesima su un archivio di milioni di righe si aspetta. Il
 * cursore invece riparte esattamente da dove si era arrivati, e ogni pagina
 * costa quanto la prima.
 *
 ...
 * servono: se torna, c'è un'altra pagina, e questa è tutta l'informazione
 * che serve davvero.
 */
 ...
 */
function componiFiltro(parametri: URLSearchParams): Prisma.RegistroRichiestaWhereInput {
...
export async function GET(richiesta: NextRequest) {
  ...
  const dove = componiFiltro(parametri);
